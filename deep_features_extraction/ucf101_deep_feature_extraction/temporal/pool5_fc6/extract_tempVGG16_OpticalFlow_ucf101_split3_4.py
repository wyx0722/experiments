...
import os
if not os.path.isfile(caffe_root + 'models/action_recognition/cuhk_action_temporal_vgg_16_split3.caffemodel'):
...
net = caffe.Net(caffe_root + 'models/action_recognition/cuhk_action_temporal_vgg_16_flow_deploy.prototxt',
                caffe_root + 'models/action_recognition/cuhk_action_temporal_vgg_16_split3.caffemodel',
                caffe.TEST)
...
rootPathSave='/home/ionut/Data/ucf101_action_temporal_vgg_16_split123_features_pool5_fc6_opticalFlow_tvL1/split3/Videos/' #the root path to save the features
...
for video in range(9990,len(videoList)):
	print ' <Split3> Features extraction for video (range(9990,len(videoList)))', video+1,':  ', videoList[video]
