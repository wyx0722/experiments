...
temp=NormalizeRowsUnit(PowerNormalization(cat(2,maxPool, multiMaxPool ), alpha));
allDist{2}=temp * temp';

temp=NormalizeRowsUnit(PowerNormalization(cat(2,NormalizeRowsUnit(vladNoMean), NormalizeRowsUnit(multiVLAD)), alpha));
allDist{3}=temp * temp';

temp=NormalizeRowsUnit(PowerNormalization(cat(2,NormalizeRowsUnit(maxPool), NormalizeRowsUnit(multiMaxPool)), alpha));
allDist{4}=temp * temp';

temp=NormalizeRowsUnit(PowerNormalization(cat(2,NormalizeRowsUnit(PowerNormalization(vladNoMean, 0.5)), NormalizeRowsUnit(PowerNormalization(multiVLAD, 0.5))), alpha));
allDist{5}=temp * temp';


temp=NormalizeRowsUnit(PowerNormalization(cat(2,NormalizeRowsUnit(PowerNormalization(maxPool, 0.5)), NormalizeRowsUnit(PowerNormalization(multiMaxPool, 0.5))), alpha));
allDist{6}=temp * temp';
