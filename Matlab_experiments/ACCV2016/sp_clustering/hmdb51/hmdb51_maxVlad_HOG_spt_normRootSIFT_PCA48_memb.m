...
t=maxVLAD_1_mean(tDesc, cell_Clusters{1}.vocabulary);
v256=zeros(length(trainTestSetPathFeatures), length(t), 'like', t); 

t=maxVLAD_1_mean(tDesc, cell_Clusters{2}.vocabulary);
v320=zeros(length(trainTestSetPathFeatures), length(t), 'like', t);

t=maxVLAD_1_mean(tDesc, cell_Clusters{3}.vocabulary);
v512=zeros(length(trainTestSetPathFeatures), length(t), 'like', t); 

t=maxVLAD_1_mean_spClustering_memb(tDesc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{1}.vocabulary);
spV8=zeros(length(trainTestSetPathFeatures), length(t), 'like', t);

t=maxVLAD_1_mean_spClustering_memb(tDesc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{2}.vocabulary);
spV32=zeros(length(trainTestSetPathFeatures), length(t), 'like', t);

t=maxVLAD_1_mean_spClustering_memb(tDesc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{3}.vocabulary);
spV64=zeros(length(trainTestSetPathFeatures), length(t), 'like', t);

t=maxVLAD_1_mean_spClustering_memb(tDesc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{4}.vocabulary);
...
parfor i=1:length(trainTestSetPathFeatures)
    ...
    v256(i, :) = maxVLAD_1_mean(desc, cell_Clusters{1}.vocabulary);
    v320(i, :) = maxVLAD_1_mean(desc, cell_Clusters{2}.vocabulary);
    v512(i, :) = maxVLAD_1_mean(desc, cell_Clusters{3}.vocabulary);
    
    spV8(i, :) = maxVLAD_1_mean_spClustering_memb(desc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{1}.vocabulary);
    spV32(i, :) = maxVLAD_1_mean_spClustering_memb(desc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{2}.vocabulary);
    spV64(i, :) = maxVLAD_1_mean_spClustering_memb(desc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{3}.vocabulary);
    spV256(i, :) = maxVLAD_1_mean_spClustering_memb(desc, cell_Clusters{1}.vocabulary, info.infoTraj(:, 8:10), cell_spClusters{4}.vocabulary);
