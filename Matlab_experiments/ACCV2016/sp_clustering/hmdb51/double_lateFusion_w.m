...
for k=1:3
    ...
    for i=1:3
        ...
        tAcc=NormalizeRows(tAcc); %make sum equal with 1
        
        clfsOut = tAcc(1) * hof_clsfOut{poz(k), i} + tAcc(2) *  hog_clsfOut{poz(k), i} + tAcc(3) * mbhx_clsfOut{poz(k), i} + tAcc(4) *  mbhy_clsfOut{poz(k), i} + tAcc(5) * spVGG19_clsfOut{poz(k), i} + tAcc(6) * tempVGG16_clsfOut{poz(k), i} ...
                    + tAcc(7) * ealryFision_all_clfsOut{poz(k), i};
