...
t_cl64=zeros(1, length(randVideos));
t_cl128=zeros(1, length(randVideos));
t_cl200=zeros(1, length(randVideos));
t_cl256=zeros(1, length(randVideos));
t_cl512=zeros(1, length(randVideos));
...
for i=1:length(randVideos)
    ...
    tic; ST_VLMPF(desc, cell_Clusters{1}.vocabulary, info.spInfo, cell_spClusters{6}.vocabulary); t_sp32cl64(i)=toc;
    tic; ST_VLMPF(desc, cell_Clusters{2}.vocabulary, info.spInfo, cell_spClusters{6}.vocabulary); t_sp32cl128(i)=toc;
    tic; ST_VLMPF(desc, cell_Clusters{3}.vocabulary, info.spInfo, cell_spClusters{6}.vocabulary); t_sp32cl200(i)=toc;
    tic; ST_VLMPF(desc, cell_Clusters{4}.vocabulary, info.spInfo, cell_spClusters{6}.vocabulary); t_sp32cl256(i)=toc;
    tic; ST_VLMPF(desc, cell_Clusters{5}.vocabulary, info.spInfo, cell_spClusters{6}.vocabulary); t_sp32cl512(i)=toc;
    ...
    tic; VLAD_1(desc, cell_Clusters{4}.vocabulary); t_v256(i)=toc;
    tic; VLAD_1(desc, cell_Clusters{5}.vocabulary); t_v512(i)=toc;
    
    
    tic; max_pooling(desc, cell_Clusters{1}.vocabulary); t_cl64(i)=toc;
    tic; max_pooling(desc, cell_Clusters{2}.vocabulary); t_cl128(i)=toc;
    tic; max_pooling(desc, cell_Clusters{3}.vocabulary); t_cl200(i)=toc;
    tic; max_pooling(desc, cell_Clusters{4}.vocabulary); t_cl256(i)=toc;
    tic; max_pooling(desc, cell_Clusters{5}.vocabulary); t_cl512(i)=toc;
...
fprintf('Average time for %d videos t_cl64: %.3f \n', length(randVideos), mean(t_cl64));
fprintf('Average time for %d videos t_cl128: %.3f \n', length(randVideos), mean(t_cl128));
fprintf('Average time for %d videos t_cl200: %.3f \n', length(randVideos), mean(t_cl200));
fprintf('Average time for %d videos t_cl256: %.3f \n', length(randVideos), mean(t_cl256));
fprintf('Average time for %d videos t_cl512: %.3f \n\n', length(randVideos), mean(t_cl512));
