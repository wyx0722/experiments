...
t=ST_VLMPF_abs(tDesc, cell_Clusters{4}.vocabulary, info.spInfo, cell_spClusters{3}.vocabulary); sp4cl256=zeros(length(allPathFeatures), length(t), 'like', t);
...
for i=1:length(allPathFeatures)%parfor i=1:length(allPathFeatures)
    ...
    sp4cl256(i, :)=ST_VLMPF_abs(desc, cell_Clusters{4}.vocabulary, info.spInfo, cell_spClusters{3}.vocabulary); 
...
temp=NormalizeRowsUnit(sp4cl256); allDist{1}=temp * temp';
