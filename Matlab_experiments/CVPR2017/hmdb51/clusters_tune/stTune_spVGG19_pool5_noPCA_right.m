...
alpha=0.5;
nPar=5;
pathFeatures='/media/HDS2-UTX/ionut/Data/hmdb51_VGG_19_features_rawFrames/Videos/'%%%%%channge~~~~~~~~~~~~
...
t=ST_VLMPF_abs(tDesc, cell_Clusters{4}.vocabulary, info.spInfo, cell_spClusters{3}.vocabulary); sp8cl256=zeros(length(allPathFeatures), length(t), 'like', t);
...
for i=1:length(allPathFeatures)%parfor i=1:length(allPathFeatures)
    ...
    sp8cl256(i, :)=ST_VLMPF_abs(desc, cell_Clusters{4}.vocabulary, info.spInfo, cell_spClusters{3}.vocabulary); 
...
temp=NormalizeRowsUnit(sp2cl256); allDist{1}=temp * temp';
temp=NormalizeRowsUnit(sp4cl256); allDist{2}=temp * temp';
temp=NormalizeRowsUnit(sp8cl256); allDist{3}=temp * temp';
temp=NormalizeRowsUnit(sp16cl256); allDist{4}=temp * temp';
temp=NormalizeRowsUnit(sp20cl256); allDist{5}=temp * temp';
temp=NormalizeRowsUnit(sp64cl256); allDist{6}=temp * temp';
temp=NormalizeRowsUnit(sp128cl256); allDist{7}=temp * temp';
temp=NormalizeRowsUnit(sp256cl256); allDist{8}=temp * temp';
