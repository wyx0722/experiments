...
m256=zeros(length(allPathFeatures), length(t), 'like', t);

t=max_pooling(tDesc, cell_Clusters{2}.vocabulary);
m319=zeros(length(allPathFeatures), length(t), 'like', t); 

t=max_pooling(tDesc, cell_Clusters{3}.vocabulary);
m512=zeros(length(allPathFeatures), length(t), 'like', t);
...
st_vlmpf32=zeros(length(allPathFeatures), length(t), 'like', t);
...
temp=NormalizeRowsUnit(PowerNormalization(m319, alpha));
allDist{5}=temp * temp';
...
temp=NormalizeRowsUnit(PowerNormalization(st_vlmpf32, alpha));
allDist{9}=temp * temp';
