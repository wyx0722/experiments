function [st_vlmpf_rep]=ST_VLMPF_abs_inner(desc, vocab, spInfo, spVocab)
...
%Calculate the similarity using inner produc
innerP = desc*vocab';%distance=distmj(desc, vocab);
[~, assign]=max(innerP, [], 2);
...
for i=1:K
    ...
    if sum(assigned)>0 % compute VLAD for each visual word (cluster) that has at least one descriptor assigned
        ...
        tDesc=desc(assigned, :);
        [~, idx]=max(abs(tDesc), [], 1);
        max_pool{i}=tDesc(sub2ind(size(tDesc), idx, 1:size(tDesc,2)));
...
spInnerP = spInfo*spVocab'; %spDistance = distmj(spInfo, spVocab);
[~, spAssign] = max(spInnerP, [], 2);
...
for i=1:size(spVocab, 1)
    ...
    if nAssigned>0
        ...
        [~, idx]=max(abs(tDesc), [], 1);
        spWordMax{i}=tDesc(sub2ind(size(tDesc), idx, 1:size(tDesc,2)));
