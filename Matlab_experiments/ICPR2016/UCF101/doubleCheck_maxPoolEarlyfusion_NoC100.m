...
temp=NormalizeRowsUnit( cat(2, NormalizeRowsUnit(PowerNormalization(maxEncode_split1_spVGG19, 0.5)), ...
                        NormalizeRowsUnit(PowerNormalization(maxEncode_split1_TempVGG16Split1, 0.5))) );
allDist{1}=temp * temp';

temp=NormalizeRowsUnit( cat(2, NormalizeRowsUnit(PowerNormalization(maxEncode_split2_spVGG19, 0.5)), ...
                        NormalizeRowsUnit(PowerNormalization(maxEncode_split2_TempVGG16Split2, 0.5))) );
allDist{2}=temp * temp';

temp=NormalizeRowsUnit( cat(2, NormalizeRowsUnit(PowerNormalization(maxEncode_split3_spVGG19, 0.5)), ...
                        NormalizeRowsUnit(PowerNormalization(maxEncode_split3_TempVGG16Split3, 0.5))) );
allDist{3}=temp * temp';
...
parfor k=1:nEncoding
    ...
    [~, clfsOut] = SvmPKOpt(trainDist, testDist, trainLabs, testLabs);
