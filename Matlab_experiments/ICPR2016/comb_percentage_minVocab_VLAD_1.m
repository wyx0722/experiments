function VLAD=comb_percentage_minVocab_VLAD_1(desc, vocab, percent)
...
[d2, vocab_assign]=sort(distance, 1, 'ascend');
...
for i=1:K
    ...
    if nAssigned>0 % compute VLAD for each visual word (cluster) that has at least one descriptor assigned
      minVocab=ceil(nAssigned * percent);
        ...
        tempDesc=desc(assigned, :);
        tempDesc=cat(1,tempDesc,desc(vocab_assign(1:minVocab, i), :));
