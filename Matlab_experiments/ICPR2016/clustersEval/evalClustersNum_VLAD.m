function [all_accuracy, all_clfsOut]=evalClustersNum_VLAD(func, mediaType, layer, network, norm, nCl, pcaD, bPathFeatures, nPar)
...
fileName=sprintf('/home/ionut/experiments/Matlab_experiments/ICPR2016/results/evalClusterDim/results_UCF50_evalClustersNum_Features%s_Layer%s_Network%s_PCAdim%d_norm%s_PNL05__VLAD.txt', descParam.MediaType,descParam.Layer, descParam.net,descParam.pcaDim, descParam.Normalisation); 
fileID=fopen(fileName, 'a');
acc0=mean(mean(cat(2, all_accuracy{1}{:})))
acc05=mean(mean(cat(2, all_accuracy{2}{:})))
acc1=mean(mean(cat(2, all_accuracy{3}{:})))
fprintf(fileID, 'Number of clusters: %d --> VLAD acc= %.4f   DA-VLAD(0.5) acc= %.4f  DA-VLAD(1) acc= %.4f \r\n', descParam.numClusters, acc0,acc05, acc1 );
fclose(fileID);



saveName = ['/home/ionut/Data/results/ICPR2016_rezults/' 'clfsOut/' 'clustersDim/'  DescParam2Name(descParam) '_VLAD_.mat'];
save(saveName, '-v7.3', 'descParam', 'all_clfsOut', 'all_accuracy');

 saveName2 = ['/home/ionut/Data/results/ICPR2016_rezults/' 'videoRep/' 'clustersDim/' DescParam2Name(descParam) '_VLAD_.mat'];
