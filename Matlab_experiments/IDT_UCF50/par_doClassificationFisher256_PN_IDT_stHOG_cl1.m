...
fprintf('Fisher Kernel extraction for %d vids: ', length(pathFeatures));
  ...
  nrCl=3;
  fisherVectors = zeros(length(vids), 2*pcaDim*numClusters*(nrCl+1));
...
parfor i=1:length(pathFeatures)
    ...
    [idx, statusEmptyCluster] = Cluster1(info.infoTraj, nrCl);
...
%     fisherV = cat(2, fisherVT{:});
    ...
    if i == 1
        descParamUsed
        if length(fisherV)~=length(fisherVectors)
            warning('length(fisherV) is not equal with length(fisherVectors)');
            size(fisherV)
            size(fisherVectors)
        end
