function  [all_accuracy, all_clfsOut]  = acc_VLAD_DV_doClassification_IDT_SP(typeFeature, normStrategy, alpha, d, cl)
...
vladVectors2_1=zeros(length(vids), 2*(size(vocabulary, 1) * size(vocabulary, 2)));
vladVectors2_2=zeros(length(vids), 2*(size(vocabulary, 1) * size(vocabulary, 2)));
vladVectors2_3=zeros(length(vids), 2*(size(vocabulary, 1) * size(vocabulary, 2)));
vladVectors2_4=zeros(length(vids), 2*(size(vocabulary, 1) * size(vocabulary, 2)));
...
parfor i=1:length(pathFeatures)
    ...
    [desc, info, descParamUsed] = MediaName2Descriptor(pathFeatures{i}, descParam, pcaMap);
        ...
        vladVectors2_1(i, :) = improvedVLAD(desc(idx(:,1), :), vocabulary, st_d, skew, nElem);
        vladVectors2_2(i, :) = improvedVLAD(desc(idx(:,2), :), vocabulary, st_d, skew, nElem);
        vladVectors2_3(i, :) = improvedVLAD(desc(idx(:,3), :), vocabulary, st_d, skew, nElem);
        vladVectors2_4(i, :) = improvedVLAD(desc(idx(:,4), :), vocabulary, st_d, skew, nElem);
...
vladVectors2=cat(2,vladVectors2_1, vladVectors2_2, vladVectors2_3, vladVectors2_4);
...
allDist{1}=n_vladVectors2 * n_vladVectors2';
...
saveName = ['/home/ionut/Data/results_desc_IDT/' 'clfsOut/' 'SpatialPyramid/' DescParam2Name(descParam) '_VLAD_DV_.mat'];
save(saveName, '-v7.3', 'descParam', 'all_clfsOut', 'all_accuracy');

 saveName2 = ['/home/ionut/Data/results_desc_IDT/' 'videoRep/' 'SpatialPyramid/' DescParam2Name(descParam) '_VLAD_DV_.mat'];
 save(saveName2, '-v7.3', 'vladVectors2');
